...
class Bot:
    ...
    def bot_Start(self):
        try:
            # creates random email
            email_len = random.randrange(9, 15)
            random_email = "".join(random.choice(string.ascii_letters)
                                   for i in range(email_len))
            random_email += "@gmail.com"
            print(f"email random: {random_email}")

            # creates random pass
            random_pass = "".join(random.choice(string.ascii_letters)
                                  for x in range(email_len))
            print(f"senha random: {random_pass}")
            ...
            # creates an account
            butao = self.driver.find_element(
                By.XPATH, '//*[@id="msite-login"]/div/div[2]/div[1]/div/ul/li[1]/div')
            butao.click()

            email = self.driver.find_element(
                By.XPATH, '//*[@id="msite-login"]/div/div[2]/div[2]/div[1]/div/div/div/div[4]/div[1]/input')
            email.send_keys(random_email)
            sleep(1)

            senha = self.driver.find_element(
                By.XPATH, '//*[@id="msite-login"]/div/div[2]/div[2]/div[1]/div/div/div/div[4]/div[2]/input')
            senha.send_keys(random_pass)
            sleep(1)

            criar_conta = self.driver.find_element(
                By, '//*[@id="msite-login"]/div/div[2]/div[2]/div[1]/div/div/div/div[4]/div[5]/a')
            criar_conta.click()
            sleep(4)
            ...
            self.driver.get(f"{self.link_aliexpress}")
            sleep(1.5)
            ajudar = self.driver.find_element_by_class_name(
                "main-action--btnWrap--3H21BlP")
            ajudar.click()
            sleep(2)
            input()
...
if __name__ == "__main__":
    ...
    while True:
        bot.bot_Start()
>>>>>>> 1e7bc86 (Updated to semi working state)
